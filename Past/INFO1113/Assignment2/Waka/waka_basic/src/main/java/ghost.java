...
//    int previousDirection;
    ...
    public ghost(mainFrame windowsFrame, int origX, int origY, int speed){
        ...
        leftUp = new message(windowsFrame.mapLeft, windowsFrame.mapUp, 0);
        rightUp = new message(windowsFrame.mapRight, windowsFrame.mapUp, 0);
        leftDown = new message(windowsFrame.mapLeft, windowsFrame.mapDown, 0);
        rightDown = new message(windowsFrame.mapRight, windowsFrame.mapDown, 0);
        this.speed = speed;
    }
    ...
    public message findTarget(int mode, message playerPosition){
        ...
        if(mode == SCATTERMODE){
            ...
            return new message(playerPosition.X / 16, playerPosition.Y / 16, 0);
        }
    }
    ...
    public double countingDistance(message nextPosition,message targetPosition){
        currentMatrixX = X / 16;
        currentMatrixY = Y / 16;
        return (int) Math.sqrt(Math.pow(targetPosition.X - nextPosition.X, 2)+Math.pow(targetPosition.Y - nextPosition.Y, 2));
    }
    ...
    public message predict(int nextWay){
        ...
        return new message(nextMatrixX, nextMatrixY, 0);
    }
    ...
    public message run(int mode, message playerPosition){
        ...
        return new message(X, Y, currentDirection);
    }
}
